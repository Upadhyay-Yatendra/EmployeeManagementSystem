...
const { sendMail } = require('../send-in-blue/index');
const saltRounds = 10;
...
exports.preSignup = async (req, res) => {
    ...
    try {
        ...
        await sendMail([{ email: employeeObj.email, name: employeeObj.name }], 'Verification | Panicle Tech', sendOTPTemplate(sixDigitOtp, 'OTP for create new account'));
...
exports.preSignupResendOTP = async (req, res) => {
    ...
    try {
        ...
        await sendMail([{ email: employee.email, name: employee.name }], 'Verification | Panicle Tech', sendOTPTemplate(sixDigitOtp, 'OTP for create new account'));
        await employee.save();
...
exports.resetPasswordSendOTP = async (req, res) => {
    ...
    try{
        ...
        await sendMail([{ email, name: employee.name }], 'Verification | Panicle Tech', sendOTPTemplate(sixDigitOtp, 'OTP for forgot password'));
